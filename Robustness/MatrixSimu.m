...
% Must be rerun after changing Rdaveg



%%%%%% add a small perturbation on M:
DM = normrnd(0,1,[d1,d2]);
DM= DM/norm(DM,2);
Mnew  =  M + d1^(0.05)*DM;
Data = CreateData(Mnew, n, 2*Rdaveg, "gaussian");
...
% Data = SDA_data.Data;
    ...
    sigHypo = randi([0,1], Rdaveg*h1*h2, 1)*2 - 1; % randi([0,1], Rdaveg*h1*h2, 1)*2 - 1;
...
for c = 1:nChunks
    ...
    parfor idx = 1:nk
        ...
        W1all = Feature(Mnew, D1,   Tk, r, signalT, "two-diff","single");
        W2all = Feature(Mnew, D2,   Tk, r, signalT, "two-diff","single");
        W3all = Feature(Mnew, Dall, Tk, r, signalT, "two-diff","double");
...
%  Dsave = load('results_weak_diff_d=1000_gam=0.1.mat');
 % FDR = Dsave.FDR;
  % POW = Dsave.POW;
   figure;
   subplot(1,2,1)
