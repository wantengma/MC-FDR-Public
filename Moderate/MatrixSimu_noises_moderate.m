...
alpha =0.1;
    ...
    p=0.2; %proportion of H1
    mu = 0.3; %var of signals
    Rdaveg = 96; % 12 48 60 96
...
seed = 111111;
rng(seed);
Data1 = CreateData(M,n,2*Rdaveg,"gaussian");
Data2 = CreateData(M,n,2*Rdaveg,"exp");
Data3 = CreateData(M,n,2*Rdaveg,"student");

% save('VarM_noise_D.mat',"Data1","Data2","Data3");
% VarM = load('VarM_noise_D.mat');
% Data1 = VarM.Data1;
% Data2 = VarM.Data2;
% Data3 = VarM.Data3;
%Null Hypothesis M=m
...
exp_type = "exp";
 ...
 % do entry comparison comparing M(1,1) with their whole rows ...
    %%%% 
    %%%% 
    %%%% 
    %%%% 
%moderate  
    ...
    for i= 1:h1
        for j = 1:h2
            T1( i+ (h1)*(j-1),1:2 ) = [i+1,j];
            T1( i+ (h1)*(j-1),3:4 ) = [1, mod(j,h2)+1 ];
        end 
    end
    % 
    [U,~,V] = svds(M,r);
    % Sigma = ProjT(U,V,T1);
    % save('S_noise_moderate.mat',"Sigma");
    Sig = load('S_noise_moderate.mat');
    ...
    X = Us*  (pinv(sqrt(S),0.0001) +0.001*eye(q))*Us.';
...
% Var = load('results_moderate_studentexp.mat');
% FDR = Var.FDR;
% POW = Var.POW;
 save(append('results_moderate_',exp_type, 'exp.mat'), 'FDR', 'POW')
...
ax = gca;
ax.FontSize = 18;
sgtitle('FDP and Power Performance under Exponential Noise', 'FontSize', 22, 'FontWeight', 'bold');
